...
```bash
# Step 7: セッションクリーンアップ
dotnet run --project tools/AgentCliTmux -- stop --session workflow-orchestrator --pane 0.0 --agent copilot
dotnet run --project tools/AgentCliTmux -- stop --session workflow-orchestrator --pane 0.1 --agent copilot --kill-session
sleep 3
