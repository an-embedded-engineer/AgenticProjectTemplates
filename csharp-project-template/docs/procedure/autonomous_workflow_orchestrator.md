...
- Agent CLI の起動、prompt 投入、出力取得、終了は `tools/AgentCliTmux` を使う
...
while true; do
  OUTPUT=$(dotnet run --project tools/AgentCliTmux -- capture --session workflow-orchestrator --pane 0.0 --lines 100 2>/dev/null)
...
dotnet run --project tools/AgentCliTmux -- send-prompt --session workflow-orchestrator --pane 0.0 --file "${PROMPT_FILE}"
```
...
```bash
REVIEW_TEXT=$(dotnet run --project tools/AgentCliTmux -- capture --session workflow-orchestrator --pane 0.1 --lines 100 2>/dev/null)
```
...
VERIFICATION_GUIDE=$(dotnet run --project tools/AgentCliTmux -- capture --session workflow-orchestrator --pane 0.0 --lines 200 \
...
dotnet run --project tools/AgentCliTmux -- stop --session workflow-orchestrator --pane 0.0 --agent codex
# Claude を終了
dotnet run --project tools/AgentCliTmux -- stop --session workflow-orchestrator --pane 0.1 --agent claude --kill-session
sleep 3
...
| ビルドまたは静的解析エラーが 2 Phase 連続で解消しない | 原因と影響範囲を提示し、方針をユーザに確認 |
...
| プロンプトが正しく入力されない | tmux paste-buffer の改行処理問題 | `dotnet run --project tools/AgentCliTmux -- send-prompt` を使い、必要なら `--submit-delay` を指定する |
