...
- プロンプトは `tmux load-buffer` + `tmux paste-buffer` で渡す（長文でも安全）
...
```bash
# 0. 既存セッションのクリーンアップ（前回の残留セッションを削除）
...
# 2. ペイン分割（左: Codex / 右: Claude 用）
tmux split-window -h -t workflow-orchestrator -c "${MAIN_PROJECT_DIR}"

# 3. Codex をペイン 0（左）で起動
#    --no-alt-screen: tmux capture-pane でスクロールバック取得を可能にする
#    -C: 作業ディレクトリを明示指定（cd より確実）
tmux send-keys -t workflow-orchestrator:0.0 \
  "codex --no-alt-screen -C ${MAIN_PROJECT_DIR} -s workspace-write -a on-request" \
  Enter

# 4. Claude をペイン 1（右）で起動
#    TERM=dumb: alt-screen を無効化して tmux capture-pane でスクロールバック取得を可能にする
#    （Claude Code CLI には --no-alt-screen オプションが存在しないため TERM=dumb で代替）
...
# '\$' は Codex のスキル呼び出し記法の '$' を出力するためのエスケープ
tmux send-keys -t workflow-orchestrator:0.0 \
"\$${SKILL}
${TRACKING_FILE}
${ITEM_ID}. ${ITEM_TITLE}
...
VERIFICATION_GUIDE=$(tmux capture-pane -t workflow-orchestrator:0.0 -p \
...
tmux send-keys -t workflow-orchestrator:0.0 "q" Enter 2>/dev/null
# Claude を終了
tmux send-keys -t workflow-orchestrator:0.1 "q" Enter 2>/dev/null
sleep 3
...
| プロンプトが正しく入力されない | tmux paste-buffer の改行処理問題 | `tmux send-keys -t workflow-orchestrator:0.1 "" Enter` で明示的にEnterを送信する |
