...
3. ソース差分レポートを生成してコミットする
   ...
   - `tools/extract_git_diff.py` を `.venv` の Python で実行する:
     ```
     .venv/bin/python tools/extract_git_diff.py \
       --commit-from <分岐元コミットID> --commit-to HEAD \
       -d src -e .py \
       -o <課題ディレクトリ>
     ```
   - 実行後、課題ディレクトリ配下には `diff.zip` と `report.md` が残る
   - `diff.zip` には `diff/` ディレクトリ一式が格納され、展開用の `diff/` ディレクトリ自体は自動削除される
   - 課題ディレクトリ配下に生成された `diff.zip` と `report.md` をコミットする
...
- ソース差分レポート（`diff.zip` + `report.md`）が課題ディレクトリに生成・コミットされている
