...
- Agent CLI の起動、prompt 投入、出力取得、終了は `~/.agentic/bin/agentic-agent-cli-tmux.sh` を使う
...
while true; do
  OUTPUT=$(~/.agentic/bin/agentic-agent-cli-tmux.sh capture --session workflow-orchestrator --pane 0.0 --lines 100 2>/dev/null)
...
~/.agentic/bin/agentic-agent-cli-tmux.sh send-prompt --session workflow-orchestrator --pane 0.0 --file "${PROMPT_FILE}"
```
...
```bash
REVIEW_TEXT=$(~/.agentic/bin/agentic-agent-cli-tmux.sh capture --session workflow-orchestrator --pane 0.1 --lines 100 2>/dev/null)
```
...
VERIFICATION_GUIDE=$(~/.agentic/bin/agentic-agent-cli-tmux.sh capture --session workflow-orchestrator --pane 0.0 --lines 200 \
...
~/.agentic/bin/agentic-agent-cli-tmux.sh stop --session workflow-orchestrator --pane 0.0 --agent codex
# Claude を終了
~/.agentic/bin/agentic-agent-cli-tmux.sh stop --session workflow-orchestrator --pane 0.1 --agent claude --kill-session
sleep 3
...
| プロンプトが正しく入力されない | tmux paste-buffer の改行処理問題 | `~/.agentic/bin/agentic-agent-cli-tmux.sh send-prompt` を使い、必要なら `--submit-delay` を指定する |
