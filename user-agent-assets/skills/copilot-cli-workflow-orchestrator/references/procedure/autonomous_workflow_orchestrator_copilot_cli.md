...
Phase 2 / 3 完了後の次 Phase への移行判断は `ask_user` でユーザ承認必須。
...
Step 4（Phase 4-a ユーザ動作確認ゲート）、Step 5（Phase 4-c master マージ）、Step 6（結果サマリー）、
...
```bash
# Step 7: セッションクリーンアップ
~/.agentic-project-templates/bin/agentic-agent-cli-tmux.sh stop --session workflow-orchestrator --pane 0.0 --agent copilot
~/.agentic-project-templates/bin/agentic-agent-cli-tmux.sh stop --session workflow-orchestrator --pane 0.1 --agent copilot --kill-session
sleep 3
