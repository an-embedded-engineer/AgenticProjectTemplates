...
```bash
# Step 7: セッションクリーンアップ
~/.agentic/bin/agentic-agent-cli-tmux.sh stop --session workflow-orchestrator --pane 0.0 --agent copilot
~/.agentic/bin/agentic-agent-cli-tmux.sh stop --session workflow-orchestrator --pane 0.1 --agent copilot --kill-session
sleep 3
