...
協調させて 6 種類の workflow を全 Phase 自律的に完了させる。
...
- Agent CLI の起動、prompt 投入、出力取得、終了は `~/.agentic-project-templates/bin/agentic-agent-cli-tmux.sh` を使う
...
2. 以下の基準でワークフロー種別を判定する
   ...
   | ドキュメントのみの作成・更新・整理 | documentation | `docs/todo/todo.md` | `documentation-workflow` |
...
1. **構造化シグナル（一次判定）**: Codex へ各 Phase 完了時に出力するよう指示する。確実に出力される場合はこちらを使う。
   ...
   [NEED_USER_VERIFICATION]    … Phase 4-a ユーザ動作確認の依頼待ち（documentation は文書最終確認）
...
2. **自然言語パターン（フォールバック判定）**: Codex が構造化シグナルを出力しない場合、Copilot が出力テキストを読んで以下のパターンで待機状態を判断する。
   ...
   | 「動作確認をお願いします」「アプリを起動して確認してください」 | Phase 4-a ユーザ動作確認の依頼待ち |
   | 「文書の最終確認をお願いします」「docs-only の確認をお願いします」 | documentation Phase 4 の文書最終確認待ち |
   | 「マージしてよいですか」「既定ブランチへのマージ承認をお願いします」 | Phase 4-c 最終マージの承認待ち |
   | 「レビューを依頼してください」「レビュー担当 Agent を起動してください」 | レビュー依頼準備完了、Claude への依頼タイミング |

```bash
# Copilot による定期読み取りのイメージ（実際は Copilot の判断ロジック）
while true; do
  OUTPUT=$(~/.agentic-project-templates/bin/agentic-agent-cli-tmux.sh capture --session workflow-orchestrator --pane 0.0 --lines 100 2>/dev/null)
...
## Step 3: レビュー Phase ループ（Phase 2 / 3）

**実行者: Copilot** が以下のループを全 Phase 完了まで繰り返す。

```
for each review Phase N in [2, 3]:
  ...
  9. [NEED_USER_VERIFICATION] を検知したら Step 4 へ（通常 workflow は動作確認ゲート、documentation は文書最終確認ゲート）
```
...
~/.agentic-project-templates/bin/agentic-agent-cli-tmux.sh send-prompt --session workflow-orchestrator --pane 0.0 --file "${PROMPT_FILE}"
```
...
```bash
REVIEW_TEXT=$(~/.agentic-project-templates/bin/agentic-agent-cli-tmux.sh capture --session workflow-orchestrator --pane 0.1 --lines 100 2>/dev/null)
```
...
## Step 4: Phase 4-a ユーザ動作確認ゲート（必須）
...
VERIFICATION_GUIDE=$(~/.agentic-project-templates/bin/agentic-agent-cli-tmux.sh capture --session workflow-orchestrator --pane 0.0 --lines 200 \
...
```bash
PROMPT_FILE="${MAIN_PROJECT_DIR}/tmp/orchestrator/codex_prompt.txt"
echo "動作確認し、問題なさそうです。Phase 4-b の完了処理へ進めてください。" > "${PROMPT_FILE}"
~/.agentic-project-templates/bin/agentic-agent-cli-tmux.sh send-prompt --session workflow-orchestrator --pane 0.0 --file "${PROMPT_FILE}"
```
...
# その後 Codex に Phase 3 差し戻しを指示
PROMPT_FILE="${MAIN_PROJECT_DIR}/tmp/orchestrator/codex_prompt.txt"
cat > "${PROMPT_FILE}" <<EOF
動作確認でNGでした。原因: <NG 詳細>
Phase 3 へ戻って修正をお願いします。
EOF
~/.agentic-project-templates/bin/agentic-agent-cli-tmux.sh send-prompt --session workflow-orchestrator --pane 0.0 --file "${PROMPT_FILE}"
# Step 3 のループに戻る
```
...
## Step 5: Phase 4-c 既定ブランチマージと最終承認
...
```
全 Phase が完了しました。既定ブランチへのマージを承認しますか？

- ブランチ: <branch名>
- 実装レビュー: 完了（open 指摘: N 件）
- completion review: <未実施 / 完了（open 指摘: M 件）>
- ユーザ動作確認: OK
```
...
```bash
git --no-pager log --oneline <default-branch> -5
...
- ブランチ: <branch名>（既定ブランチにマージ済み）
- 完了 Phase: Phase 0 〜 Phase 4
- 最終コミット: <hash>
...
~/.agentic-project-templates/bin/agentic-agent-cli-tmux.sh stop --session workflow-orchestrator --pane 0.0 --agent codex
# Claude を終了
~/.agentic-project-templates/bin/agentic-agent-cli-tmux.sh stop --session workflow-orchestrator --pane 0.1 --agent claude --kill-session
sleep 3
...
| Phase 4-a ユーザ動作確認が **NG** | NG 詳細を共有し、Phase 3 差し戻しを報告 |
| 項目の要件が曖昧で種別判定不能 | 5 workflow のどれで扱うかをユーザに確認 |
| Codex ペインが無応答（10 分超え） | セッション再起動の要否をユーザに確認 |
| 既定ブランチマージでコンフリクト発生 | コンフリクト箇所を提示し、解消方法をユーザに確認 |
| 検証コマンドが 2 Phase 連続で通らない | 原因と影響範囲を提示し、方針をユーザに確認 |
...
| Phase 4-a 動作確認ゲート | Copilot からの通知に従いアプリを起動して確認し、OK / NG を回答 |
| Phase 4-c 最終マージ | Copilot からの承認確認に OK / 保留を回答 |
...
| プロンプトが正しく入力されない | tmux paste-buffer の改行処理問題 | `~/.agentic-project-templates/bin/agentic-agent-cli-tmux.sh send-prompt` を使い、必要なら `--submit-delay` を指定する |
