...
description: AgenticProjectTemplates のレビュー依頼を Claude Code CLI のインタラクティブモードで自動化する手順。Codex が ~/.agentic/bin/agentic-agent-cli-tmux.sh 経由で Claude を起動・監視し、spec-change/new-feature/bugfix/issue-resolution/refactoring の Phase 2/3/4/5 レビューと指摘対応確認を代行する。
---
...
4. Codex が `~/.agentic/bin/agentic-agent-cli-tmux.sh` を実行できる
...
- 一時ファイル例: `/tmp/agentic_claude_review_prompt.txt`
- 長文は `~/.agentic/bin/agentic-agent-cli-tmux.sh send-prompt` で送る

4. Claude にレビューを依頼する
```bash
PROMPT_FILE="/tmp/agentic_claude_review_prompt.txt"
...
- プロンプト投入が崩れる: `tmux send-keys` の直打ちではなく `~/.agentic/bin/agentic-agent-cli-tmux.sh send-prompt` を使う
