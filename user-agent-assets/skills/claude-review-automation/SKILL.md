...
description: AgenticProjectTemplates のレビュー依頼を Claude Code CLI のインタラクティブモードで自動化する手順。Codex が ~/.agentic-project-templates/bin/agentic-agent-cli-tmux.sh 経由で Claude を起動・監視し、spec-change/new-feature/bugfix/issue-resolution/refactoring の Phase 2/3/4/5 レビューと指摘対応確認を代行する。
---
...
- 例: `agentic-project-templates-claude-<topic>`
- 同じ issue の再レビューでは同じセッションを再利用する

2. Claude はインタラクティブモードで起動する
```bash
SESSION_NAME="agentic-project-templates-claude-${TOPIC}"
