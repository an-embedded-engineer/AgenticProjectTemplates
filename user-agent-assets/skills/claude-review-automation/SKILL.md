...
description: AgenticProjectTemplates のレビュー依頼を Claude Code CLI のインタラクティブモードで自動化する手順。Codex が ~/.agentic-project-templates/bin/agentic-agent-cli-tmux.sh 経由で Claude を起動・監視し、spec-change/new-feature/bugfix/issue-resolution/refactoring/documentation の Phase 2/3 レビューと optional completion review の指摘対応確認を代行する。
---
...
- `spec-change` / `new-feature` / `bugfix` / `issue-resolution` / `refactoring` / `documentation` の Phase 2 / 3 と optional completion review で review 文書作成と follow-up 確認を回したい時

## 実行ルール（索引）

- workflow 選択: workflow 判定ルール
- 仕様変更: `references/procedure/spec_change_workflow.md`
- 新機能追加: `references/procedure/new_feature_workflow.md`
- 不具合修正: `references/procedure/bugfix_workflow.md`
- 課題解決: `references/procedure/issue_resolution_workflow.md`
...
- tmux 監視の基準手順: `references/procedure/autonomous_workflow_orchestrator.md`
...
4. Codex が `~/.agentic-project-templates/bin/agentic-agent-cli-tmux.sh` を実行できる
...
- `docs/design_analysis/documentation/` 配下なら `documentation`
- `docs/todo/todo.md` の追跡項目に `workflow: documentation` が明記されているなら `documentation`
- 判定不能時は現在進行中の workflow skill（`spec-change-workflow` / `new-feature-workflow` / `bugfix-workflow` / `issue-resolution-workflow` / `refactoring-workflow` / `documentation-workflow`）に合わせる
...
- 一時ファイル例: `/tmp/agentic-project-templates_claude_review_prompt.txt`
- 長文は `~/.agentic-project-templates/bin/agentic-agent-cli-tmux.sh send-prompt` で送る

4. Claude にレビューを依頼する
```bash
PROMPT_FILE="/tmp/agentic-project-templates_claude_review_prompt.txt"
...
レビュー観点は `ai-review-response-workflow` skill に同梱された `references/procedure/review_checkpoints.md` と現在の workflow 文書に従ってください。
```
...
6. Phase 2 / 3 / optional completion の review 文書名が命名規則に一致していることを確認する
...
- プロンプト投入が崩れる: `tmux send-keys` の直打ちではなく `~/.agentic-project-templates/bin/agentic-agent-cli-tmux.sh send-prompt` を使う
