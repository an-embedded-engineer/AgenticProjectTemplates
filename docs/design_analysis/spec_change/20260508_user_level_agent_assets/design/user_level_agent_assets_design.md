...
5. user-level assets 利用前提
  - user-level skill を前提にすること、fallback がある場合の参照先
...
- `research-analysis-workflow`、`ai-review-response-workflow`、`claude-review-automation`、`autonomous-workflow-orchestrator`、`copilot-cli-workflow-orchestrator` は、Copilot Chat の日常導線より Claude / Codex / 明示起動用途が主であり、smoke test 不合格時も `.github/skills/` fallback 必須対象に含めない
- `.claude/skills/` と `.agents/skills/` の workspace fallback は新規増設しない
- fallback は team-shared workspace 互換経路としてのみ扱う

smoke test 不合格時の代替案内:

- `ai-review-response-workflow` と `claude-review-automation` は Claude / Codex 側 user-level skill から起動する
- orchestration skill は user-level install 済み環境、または root 側から明示参照する advanced operation として扱う
