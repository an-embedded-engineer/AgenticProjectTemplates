...
- current phase の project-level 導線は `project-doc-bootstrap` を起点とし、target project へ docs 雛形、`instructions/agent_common_master.md`、`instructions/agent_sync_guide.md`、`scripts/sync_agent_instructions.*` をコピーして成立させる
- repo 直下および既存 template の project-level files は、移行完了と smoke test 完了まで変更しない
...
- `instructions/agent_sync_guide.md`
  - 役割: `instructions/agent_common_master.md` から生成物 3 種を再生成する手順
- `scripts/sync_agent_instructions.*`
  - 役割: `instructions/agent_common_master.md` から `AGENTS.md` / `CLAUDE.md` / `.github/copilot-instructions.md` を再生成する
...
- 責務
  - bootstrap 後の target project で、`instructions/agent_common_master.md` から生成物 3 種を再生成する
  - current phase では `scripts/sync_agent_instructions.*` を target project に同梱する
- 非責務
...
current phase では、`project-doc-bootstrap` から target project へ `scripts/sync_agent_instructions.*` を同梱して使う。

責務は次へ縮小する。

- `instructions/agent_common_master.md` から `AGENTS.md` / `CLAUDE.md` / `.github/copilot-instructions.md` を再生成する
- user home 配下には一切書き込まない

移行完了後に、repo 直下 / template 側の script rename や thin index 化を follow-up として扱う。
...
- suite skill には `workflow_phase_library` を追加配置しない
...
1. ユーザが新規プロジェクトを作成、または既存プロジェクトを開く
2. `project-doc-bootstrap` を skill として実行する
3. Agent が docs 雛形と project-level `instructions/agent_common_master.md`、`instructions/agent_sync_guide.md`、`scripts/sync_agent_instructions.*` を target project へコピーする
4. Agent が copied docs / instructions の placeholder を記入する
5. target project で `scripts/sync_agent_instructions.*` を実行し、`AGENTS.md`、`CLAUDE.md`、`.github/copilot-instructions.md` を再生成する
...
`docs/rules/skill_catalog.md` の削除 ripple は、repo 直下 / 既存 template の移行完了後に follow-up として扱う。対象は少なくとも次である。
...
5. `project-doc-bootstrap` から target project へ project-level instructions / sync script を初期配置
6. root / template sync source 薄化と `skill_catalog.md` ripple は移行完了後の follow-up
...
- repo 直下および既存 template の project-level files は、移行完了まで変更しない
...
### 12.2 current phase の更新対象

- `user-agent-assets/skills/project-doc-bootstrap/**`
- `docs/design_analysis/spec_change/20260508_user_level_agent_assets/**`

### 12.3 移行完了後の follow-up 更新対象
...
2. project-level instructions と sync script の bootstrap 導線、および移行完了後の責務縮小方針が明示されている。
...
5. `skill_catalog.md` 削除 ripple の follow-up 対象が列挙されている。
...
- current phase では `project-doc-bootstrap` から project-level `agent_common_master.md` / `agent_sync_guide.md` / `sync_agent_instructions.*` を target project へコピーして導線を成立させる
- repo 直下および既存 template の project-level files は、移行完了後の follow-up まで現状維持とする
