...
その後の `pwsh 7.6.1` 環境での追加確認により、PowerShell 版の installer dry-run、bootstrap、sync script も成功した。
...
| 追加2. bootstrap の placeholder scan 範囲不足 | 対応済み | scan 対象を `docs` に加えて `instructions` と既存生成物 3 種へ拡張し、sync 後の再 scan ガイドも追加した |
| 追加3. PowerShell sync script の `param` 位置 | 対応済み | `sync_agent_instructions.ps1` の `param(...)` を script 先頭へ移動し、PowerShell script としての parameter binding に沿う形へ修正した |

備考:

- `pwsh 7.6.1` で PowerShell wrapper / installer の実行確認済み
...
- `copy_doc_templates.sh` / `.ps1` の placeholder scan は `docs/` に加えて `instructions/` と、存在する場合の `AGENTS.md` / `CLAUDE.md` / `.github/copilot-instructions.md` も対象に拡張した
- placeholder scan の末尾に、sync 実行後の wrapper 再実行で生成済み Agent 向けファイルも再 scan されるガイドを追加した
- `project-doc-bootstrap/SKILL.md` は「docs / instructions の placeholder 一覧を確認する」と明記し、wrapper 出力との整合を取った
- `sync_agent_instructions.ps1` の `param(...)` は shebang / コメント直後へ移動した
- `agent_sync_guide.md` に「sync 実行時は生成物 3 種を上書きする」契約を追記した
- `pwsh 7.6.1` で `install_user_agent_assets.ps1 -DryRun`、`copy_doc_templates.ps1 -Language python`、生成先 `sync_agent_instructions.ps1 -Help/-All`、sync 後の `copy_doc_templates.ps1` 再 scan が成功した
