...
3. Copilot agent skills の対象環境を確認する
   ...
   - 実装前確認対象: 利用中バージョンで `~/.copilot/skills` または `~/.agents/skills` が実際に検出されるか
...
次 workflow へ渡す前提として、実装前に解消すべき不明点は本節へ集約する。リスク節や変更手順案に出てくる確認事項も、実装判断が必要なものはここで追跡する。
...
2. Copilot の対象環境
   ...
   - GitHub Docs 上は Copilot CLI / Visual Studio Code / agent mode で agent skills が動作する
   - 対象 PC の Copilot Chat / Copilot CLI バージョンで `~/.copilot/skills` または `~/.agents/skills` が検出されるか
...
8. `agent_cli_tmux` / `AgentCliTmux` の OS 別 wrapper 方針
   - `docs/design_analysis/research_analysis/20260501_cli_auto_approval_settings/report.md` で追記した wrapper 方針を取り込むか
   - macOS / Linux は `.sh`、Windows は `.ps1`、必要なら `.cmd` を提供するか
   - 承認設定では `python` / `dotnet` 全体ではなく、用途別 wrapper command を allowlist する方針にするか
   - PowerShell / sh の引数エスケープと destructive command を通さない制約をどう設計するか
