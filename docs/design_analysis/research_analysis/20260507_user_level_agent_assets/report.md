...
- Copilot の user-level skill ディレクトリは、今回確認した GitHub Docs では明示確認できなかった。Copilot については当面、repo-level `.github/copilot-instructions.md` / `.github/instructions/*.instructions.md` / `AGENTS.md` を残す方が安全である。spec-change では、Copilot 向けに従来どおり project-level instructions を厚めに残す案と、薄い `AGENTS.md` から user-level docs を間接参照する案を比較して決める必要がある。
...
5. `install_user_agent_assets.*` を作り、少なくとも Claude / Codex へ install できるようにする
6. install script は `~/.claude/skills` が未作成でも作成でき、既存 user-level skill を不用意に上書きしない冪等な挙動にする
...
- spec-change では次のどちらを採るか決める
  - 案 A: Copilot 向け project-level instructions は従来より厚めに残し、user-level skills 正本化の対象外にする
  - 案 B: 薄い `AGENTS.md` から user-level docs / installed skills の利用を案内し、project-level は索引だけにする
...
2. Copilot の参照経路
   - `.github/copilot-instructions.md` / `.github/instructions/*.instructions.md` を厚めに残すか
   - `AGENTS.md` を user-level 資産への索引として使うか
...
5. Copilot は user-level skill 依存を避け、当面 repo-level instructions を残す。ただし、`AGENTS.md` を介した user-level 資産参照を採るかどうかは spec-change で決める。
