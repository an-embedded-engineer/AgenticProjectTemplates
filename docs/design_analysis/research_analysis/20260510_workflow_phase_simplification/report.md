...
branch を Phase / topic 単位で分けている前提では、途中 Phase の hash は `git log` と review 文書から復元できる。そのため、`related_commits` は毎 Phase 必須更新ではなく、完了処理でまとめて記載する運用へ寄せるのがよい。
...
`meta.md` については、途中 Phase の `related_commits` 即時更新を標準から外し、completion Phase でまとめて記録する方針を README または common phase library に明記する。
...
| 大規模案件で review 粒度が粗くなる | plan/design 統合によりレビュー対象が大きくなる | core workflow に戻さず、WBS 分解 skill で複数 work package に分ける |
...
| `related_commits` 更新 commit が増える | commit hash 確定後に meta 追記 commit が必要になる | completion Phase で主要 commit をまとめて記録する |
...
7. `related_commits` を completion Phase でまとめる運用へ common phase library と review 観点を更新する
8. WBS 分解用 skill を追加するか、少なくとも次フェーズの設計対象として起票する
9. user-level assets install / sync の hydrate 結果で common phase library が正しく配布されることを確認する
10. 小さな文書変更タスクで smoke test を行い、review 文書が design / impl の 2 本で回ることを確認する
...
`meta.md` の `related_commits` は、各 Phase で即時更新せず、completion Phase でまとめて記録する案が妥当である。branch が topic 単位で分かれているため、途中の hash は `git log` と review 文書から追跡でき、meta 追記だけの commit を減らせる。
...
1. WBS 分解用 skill を新設するか、既存 `research-analysis-workflow` の派生として扱うか
...
7. `related_commits` に記録する commit の粒度を、全 commit、Phase 境界 commit、review/follow-up commit、最終 report commit のどこまでにするか
