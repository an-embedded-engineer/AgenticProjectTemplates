...
```bash
# Step 7: セッションクリーンアップ
python scripts/agent_cli_tmux.py stop --session workflow-orchestrator --pane 0.0 --agent copilot
python scripts/agent_cli_tmux.py stop --session workflow-orchestrator --pane 0.1 --agent copilot --kill-session
sleep 3
