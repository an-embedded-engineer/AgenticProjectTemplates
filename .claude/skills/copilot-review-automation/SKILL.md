...
| レビュー Agent | Copilot CLI（デフォルト: `claude-sonnet-4.6`） | `scripts/agent_cli_tmux.py` 経由で起動 |
...
4. 設計・実装 Agent が `scripts/agent_cli_tmux.py` を実行できる
...
```bash
python scripts/agent_cli_tmux.py stop \
  --session "${SESSION_NAME}" \
  --pane 0.0 \
  --agent copilot \
  --kill-session
